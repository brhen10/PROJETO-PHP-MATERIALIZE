...
if ($acao == 'logon') {
    if ($login->logon($_POST['login'], $_POST['senha'])) {
        header("Location:../index_1.php");
        die();
    } else {
        $msgErro = "login / senha invalidos";
        $acao = '';
    }
}
if ($acao == '') {
    if (isset($_POST['origem'])) {
        $paginaOrigem = $_POST['origem'];
    } else {
...
input:focus { box-shadow: inset 0 -5px 45px rgba(100,100,100,0.4), 0 1px 1px rgba(255,255,255,0.2); }
  ...
  <body>
        <body>
            ...
            <?php
            if (isset($msgErro)) {
                ?><h2 style="color:#f00;"><?= $msgErro?></h2><?php
            }
            ?>
                <div class="login">
            <form method="POST" action="world-autenticacao.php?acao=logon">
